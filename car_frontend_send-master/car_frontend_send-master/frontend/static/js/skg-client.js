...
function skg_quantization() {
	alert_info("Quantization...")
    socket.emit("skg_quantization", {
    }, (resp) => {
	if (resp.status != "ok") {
	    alert_error(resp.explain);
	} else {
	    alert_info("Quantization Done!");
	}
    })
}

function skg_conciliation() {
    socket.emit("skg_conciliation", {
    }, (resp) => {
	if (resp.status != "ok") {
	    alert_error(resp.explain);
	} else {
	    alert_info("Conciliation!");
	}
    })
}

function skg_amplification() {
    socket.emit("skg_amplification", {
    }, (resp) => {
	if (resp.status != "ok") {
	    alert_error(resp.explain);
	} else {
	    alert_info("Amplification!");
	}
    })
}

function skg_encryption() {
    socket.emit("skg_encryption", {
    }, (resp) => {
	if (resp.status != "ok") {
	    alert_error(resp.explain);
	} else {
	    alert_info("Encryption!");
	}
    })
}

function skg_autorun() {
	const addr = get_element_value_or("usrp-server-addr-form", "");
    if (addr == "") {
	alert_error("Please specify 'USRP Server Address'");
	return;
    ...
    socket.emit("skg_autorun", {
		usrp_server_addr: "tcp://" + addr,
    }, (resp) => {
	if (resp.status != "ok") {
	    alert_error(resp.explain);
	} else {
	    alert_info("Autorun Started!");
	}
    })
}
