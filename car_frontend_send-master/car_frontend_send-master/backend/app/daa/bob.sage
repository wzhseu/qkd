import json
from socket_client import socket_from  
...
while True:
    msg="Bob ask to communicate with Alice..."
    print("Bob send: ", msg)
...
####gen
    ...
    print("gen done")
    msg="Bob generate the parameter"
    print("Bob send: ", msg)
    client.send(msg.encode('utf-8'))#将发送的数据进行编码
    a=client.recv(1024)#接受服务端的信息，最大数据为1k
    print(a.decode('utf-8'))    
...
#print("z_5:",z_5)
    ...
    msg="Bob send its sig to Alice"
    print("Bob send: ", msg)
    client.send(msg.encode('utf-8'))#将发送的数据进行编码
    a=client.recv(1024)#接受服务端的信息，最大数据为1k
    print(a.decode('utf-8'))

    user('192.168.201.131', 6969,W_1,W_1_5,W_2_5,t_1,t_2,c,a_1_T,a_1_T_sig_5,a_2_T,a_2_T_sig_5,z,z_5)
    
    msg="Bob sending over."
    print("Bob send: ", msg)
    client.send(msg.encode('utf-8'))#将发送的数据进行编码
    a=client.recv(1024)#接受服务端的信息，最大数据为1k
    print(a.decode('utf-8'))    
...
#### Alice send data to Bob
    W_1,W_1_5,W_2_5,t_1,t_2,c,a_1_T,a_1_T_sig_5,a_2_T,a_2_T_sig_5,z,z_5=server('192.168.201.131', 6970);  # 注意是本机地址
    ...
    msg="Bob received Alice's verification data"
    print("Bob send: ", msg)
    client.send(msg.encode('utf-8'))#将发送的数据进行编码
    a=client.recv(1024)#接受服务端的信息，最大数据为1k
    print(a.decode('utf-8'))
