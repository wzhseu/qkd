...
###########################################################################################
#此PY文件实现了国产SM3加密算法，因为SM3算法现在并不能通过调用库来实现，所以去copy了一份别人实现过的算法代码；
#SM3算法的输出是一个256比特的摘要，官方宣称SM3算法的安全性与SHA-256相当；
#要使用SM3算法，调用SM3.py中的sm3_hash()函数即可，如果输入是bytes的形式，即输入为b'\x61\x62\x63'这样的，
#那么直接作为输入即可，即sm3_hash(b'\x61\x62\x63')；此代码还支持十六进制形式的输入，即输入为'616263'这样，
#那么输入sm3_hash('616263'，1)也可以处理。
###########################################################################################
